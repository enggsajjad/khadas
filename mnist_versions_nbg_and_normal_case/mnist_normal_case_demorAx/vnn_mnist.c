...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /*-----------------------------------------
      ...
      output    - [24, 24, 200, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[0], VSI_NN_OP_CONV2D, 3, 1, 11);
    ...
    /*-----------------------------------------
      ...
      output    - [24, 24, 200, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[1], VSI_NN_OP_RELU, 1, 1, 10);

    /*-----------------------------------------
      lid       - MaxPool_MaxPool_2_9
      var       - node[2]
      ...
      output    - [12, 12, 200, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[2], VSI_NN_OP_POOL, 1, 1, 9);
    ...
    /*-----------------------------------------
      ...
      output    - [8, 8, 500, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[3], VSI_NN_OP_CONV2D, 3, 1, 8);
    node[3]->nn_param.conv2d.ksize[0] = 5;
    node[3]->nn_param.conv2d.ksize[1] = 5;
    node[3]->nn_param.conv2d.weights = 500;
    ...
    /*-----------------------------------------
      ...
      output    - [8, 8, 500, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[4], VSI_NN_OP_RELU, 1, 1, 7);

    /*-----------------------------------------
      lid       - MaxPool_MaxPool_5_6
      var       - node[5]
      name      - MaxPool_MaxPool_5
      operation - pooling
      input     - [8, 8, 500, 1]
      output    - [4, 4, 500, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[5], VSI_NN_OP_POOL, 1, 1, 6);
    ...
    /*-----------------------------------------
      ...
      output    - [8000, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[6], VSI_NN_OP_RESHAPE, 1, 1, 5);
    node[6]->nn_param.reshape.size = shape_1;
    node[6]->nn_param.reshape.dim_num = 2;
    ...
    /*-----------------------------------------
      ...
      output    - [5000, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[7], VSI_NN_OP_FCL, 3, 1, 4);
    node[7]->nn_param.fcl.weights = 5000;
    ...
    /*-----------------------------------------
      ...
      output    - [5000, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[8], VSI_NN_OP_RELU, 1, 1, 3);
    ...
    /*-----------------------------------------
      ...
      output    - [10, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[9], VSI_NN_OP_FCL, 3, 1, 2);
    node[9]->nn_param.fcl.weights = 10;
    ...
    /*-----------------------------------------
      lid       - LogSoftmax_LogSoftmax_12_1
      var       - node[10]
      name      - LogSoftmax_LogSoftmax_12
      operation - log_softmax
      input     - [10, 1]
      output    - [10, 1]
    -----------------------------------------*/
    NEW_VXNODE(node[10], VSI_NN_OP_LOG_SOFTMAX, 1, 1, 1);
    node[10]->nn_param.log_softmax.betaValue = 1.0;
    node[10]->nn_param.log_softmax.axis = 0;
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /* @attach_LogSoftmax_LogSoftmax_12/out0_0:out0 */
    attr.size[0] = 10;
    attr.size[1] = 1;
    attr.dim_num = 2;
    attr.dtype.scale = 0.02607259713113308;
    ...
    /* @Conv_Conv_0_11:weight */
    attr.size[0] = 5;
    attr.size[1] = 5;
    attr.size[2] = 3;
    attr.size[3] = 200;
    attr.dim_num = 4;
    attr.dtype.scale = 0.0013285926543176174;
    ...
    attr.size[0] = 200;
    attr.dim_num = 1;
    attr.dtype.scale = 5.21016757995779e-06;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[1], attr, VSI_NN_TYPE_INT32, 0, 800);

    /* @Conv_Conv_3_8:weight */
    attr.size[0] = 5;
    attr.size[1] = 5;
    attr.size[2] = 200;
    attr.size[3] = 500;
    attr.dim_num = 4;
    attr.dtype.scale = 0.0002720515476539731;
    attr.dtype.zero_point = 115;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[2], attr, VSI_NN_TYPE_UINT8, 17800, 2500000);

    /* @Conv_Conv_3_8:bias */
    attr.size[0] = 500;
    attr.dim_num = 1;
    attr.dtype.scale = 1.1900969419214842e-06;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[3], attr, VSI_NN_TYPE_INT32, 15800, 2000);

    /* @Gemm_Gemm_9_4:weight */
    attr.size[0] = 8000;
    attr.size[1] = 5000;
    attr.dim_num = 2;
    attr.dtype.scale = 0.0001544763654237613;
    attr.dtype.zero_point = 138;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[4], attr, VSI_NN_TYPE_UINT8, 2587840, 40000000);

    /* @Gemm_Gemm_9_4:bias */
    attr.size[0] = 5000;
    attr.dim_num = 1;
    attr.dtype.scale = 7.850342218717829e-07;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[5], attr, VSI_NN_TYPE_INT32, 2567840, 20000);

    /* @Gemm_Gemm_11_2:weight */
    attr.size[0] = 5000;
    attr.size[1] = 10;
    attr.dim_num = 2;
    attr.dtype.scale = 0.0005996805266477168;
    attr.dtype.zero_point = 121;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[6], attr, VSI_NN_TYPE_UINT8, 2517840, 50000);

    /* @Gemm_Gemm_11_2:bias */
    attr.size[0] = 10;
    attr.dim_num = 1;
    attr.dtype.scale = 1.9717522985794687e-06;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[7], attr, VSI_NN_TYPE_INT32, 2517800, 40);



    /* @Conv_Conv_0_11:out0 */
    attr.dtype.scale = 0.004374527372419834;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    ...
    /* @Conv_Conv_3_8:out0 */
    attr.dtype.scale = 0.005081905052065849;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    ...
    /* @Gemm_Gemm_9_4:out0 */
    attr.dtype.scale = 0.003288004547357559;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    ...
    /* @Gemm_Gemm_11_2:out0 */
    attr.dtype.scale = 0.024919094517827034;
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    node[10]->output.tensors[0] = norm_tensor[0];
