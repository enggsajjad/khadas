...
#define NET_NODE_NUM            (3)
#define NET_NORM_TENSOR_NUM     (2)
#define NET_CONST_TENSOR_NUM    (1)
...
vsi_nn_graph_t * vnn_CreateNopnet
    (
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /*-----------------------------------------
      lid       - Initializer_onnx//Add_6_3
      var       - node[0]
      name      - Initializer_onnx//Add_6
      operation - variable
      input     - 
      output    - [2352]
    -----------------------------------------*/
    NEW_VXNODE(node[0], VSI_NN_OP_VARIABLE, 1, 1, 3);

    /*-----------------------------------------
      lid       - Reshape_Reshape_5_2
      var       - node[1]
      name      - Reshape_Reshape_5
      operation - reshape
      input     - [28, 28, 3, 1]
      output    - [2352]
    -----------------------------------------*/
    NEW_VXNODE(node[1], VSI_NN_OP_RESHAPE, 1, 1, 2);
    node[1]->nn_param.reshape.size = shape_1;
    node[1]->nn_param.reshape.dim_num = 1;
    ...
    /*-----------------------------------------
      ...
      output    - [2352]
    -----------------------------------------*/
    NEW_VXNODE(node[2], VSI_NN_OP_ADD, 2, 1, 1);
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    if( !inference_with_nbg )
    {
    /* @Initializer_onnx//Add_6_3:data */
    attr.size[0] = 2352;
    attr.dim_num = 1;
    attr.dtype.scale = 1.0;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_CONST_TENSOR(const_tensor[0], attr, VSI_NN_TYPE_UINT8, 0, 2352);



    /* @Initializer_onnx//Add_6_3:out0 */
    attr.dtype.scale = 1.0;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_VIRTUAL_TENSOR(node[0]->output.tensors[0], attr, VSI_NN_TYPE_UINT8);

    /* @Reshape_Reshape_5_2:out0 */
    attr.dtype.scale = 1.0;
    attr.dtype.zero_point = 0;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    node[2]->output.tensors[0] = norm_tensor[0];

    /* Initializer_onnx//Add_6_3 */
    ...
    node[2]->input.tensors[1] = node[0]->output.tensors[0];
...
error:
    ...
    vnn_ReleaseNopnet( graph, release_ctx );

    return NULL;
} /* vsi_nn_CreateNopnet() */

void vnn_ReleaseNopnet
    (
    vsi_nn_graph_t * graph,
    vsi_bool release_ctx
    )
