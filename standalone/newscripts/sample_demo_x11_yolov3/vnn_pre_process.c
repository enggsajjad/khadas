...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.scale = 0.003906;
    MY_DBG("load_input_meta 4\n");
...
static uint8_t * _handle_multiple_inputs_nbg
    (
...
vsi_status vnn_PreProcessYolov3_ImageProcess_nbg
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
...
uint8_t *  vnn_PreProcessYolov3_nbg
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    status = VSI_FAILURE;
    MY_DBG("vnn_PreProcessYolov3_nbg1\n");
    for(i = 0; i < input_num; i++)
    {
        MY_DBG("Input %d is %s\n",i,inputs[i]);
    }
    MY_DBG("Graph input %d\n",graph->input.num);
    ...
    {
        //status = _handle_multiple_inputs(graph, i, inputs[i]);
		MY_DBG("vnn_PreProcessYolov3_nbg2\n");
		data = _handle_multiple_inputs_nbg(graph, i, inputs[i]);
		MY_DBG("Input %d is %s\n",i,inputs[i]);
        //TEST_CHECK_STATUS(status, final);//Sajjad gives error
    }
