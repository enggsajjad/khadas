...
{
    return vnn_PostProcessNopnet( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessNopnet_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessNopnet( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateNopnet( data_file_name, NULL,
