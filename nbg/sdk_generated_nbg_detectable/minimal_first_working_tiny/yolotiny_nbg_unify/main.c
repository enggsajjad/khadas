...
#include "yolo_tiny_process.h"//Sajjad
/*-------------------------------------------
...
{
    vnn_ReleaseYolotiny( graph, TRUE );
...
{
    return vnn_PostProcessYolotiny( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessYolotiny_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessYolotiny( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateYolotiny( data_file_name, NULL,
    ...
    )
{
    ...
    yolo_tiny_postprocess(graph, &resultData);
