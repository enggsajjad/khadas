...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.mean[0] = 127.0;
    input_meta_tab[0].image.mean[1] = 127.0;
    input_meta_tab[0].image.mean[2] = 127.0;
    ...
    )
{
    ...
    printf("\n+++++++++++++ fdata[0] after mean: %f\n", fdata[0]);
    printf("\n+++++++++++++ fdata[0] means: %f %f %f\n", meta->image.mean[0], meta->image.mean[1], meta->image.mean[2]);
...
*/
vsi_status vnn_PreProcessMnist_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessMnist_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessMnist
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
