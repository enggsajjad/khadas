    ...
    )
{
	...
	printf("_get_jpeg_data:1\n");
    bmpData = _decode_jpeg(filename, tensor);
	printf("_get_jpeg_data:2\n");
    TEST_CHECK_PTR(bmpData, final);
    ...
    )
{
    ...
    data = NULL;
    tensor = NULL;
	printf("_handle_multiple_inputs:1\n");
    memset(&meta, 0, sizeof(vnn_input_meta_t));
	printf("_handle_multiple_inputs:2\n");
    tensor = vsi_nn_GetTensor( graph, graph->input.tensors[idx] );
	printf("_handle_multiple_inputs:3\n");
    meta = input_meta_tab[idx];
    fileType = _get_file_type(input_file);
	printf("_handle_multiple_inputs:4\n");
    switch(fileType)
    {
    case NN_FILE_JPG:
		printf("_handle_multiple_inputs:5\n");
        data = _get_jpeg_data(tensor, &meta, input_file);
		printf("_handle_multiple_inputs:6\n");
        TEST_CHECK_PTR(data, final);
        break;
    case NN_FILE_TENSOR:
    ...
    )
{
    ...
    status = VSI_FAILURE;
	printf("vnn_PreProcessYolov3:1\n");
    _load_input_meta();
	printf("vnn_PreProcessYolov3:2\n");
