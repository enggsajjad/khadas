...
{
    vnn_ReleaseMobilenetTf( graph, TRUE );
...
{
    return vnn_PostProcessMobilenetTf( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessMobilenetTf_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessMobilenetTf( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateMobilenetTf( data_file_name, NULL,
