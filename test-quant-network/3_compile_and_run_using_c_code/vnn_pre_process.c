...
*/
    data = bmpData; //_float32_to_dtype(fdata, tensor);
    TEST_CHECK_PTR(data, final);
...
*/
vsi_status vnn_PreProcessNopnet_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessNopnet_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessNopnet
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
